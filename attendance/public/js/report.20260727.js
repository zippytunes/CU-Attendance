...
async function loadData() {
  const res = await fetch("data/report.json?v=20260727", { cache: "no-store" });
...
function renderYearExplorer(data) {
  ...
  document.getElementById("year-hour-title").textContent = `Breakdown by service — ${state.year}`;
  ...
  destroyChart("yearWeekly");
  state.charts.yearWeekly = new Chart(document.getElementById("chart-year-weekly"), {
    type: "line",
    data: {
  ...
  destroyChart("yearHours");
  state.charts.yearHours = new Chart(document.getElementById("chart-year-hours"), {
    type: "bar",
    data: {
      labels: [
        "9 AM Traditional",
...
function renderMonthExplorer(data) {
  ...
  `;
  document.getElementById("month-weekly-title").textContent = `Sundays — ${m.month_name} ${m.year}`;
  document.getElementById("month-hour-title").textContent =
    `Breakdown by service — ${mh.month_name} ${mh.year}`;
  ...
  destroyChart("monthWeekly");
  state.charts.monthWeekly = new Chart(document.getElementById("chart-month-weekly"), {
    type: "bar",
  ...
  destroyChart("monthHours");
  state.charts.monthHours = new Chart(document.getElementById("chart-month-hours"), {
    type: "bar",
    data: {
...
function renderRhythm(data) {
  ...
  destroyChart("rhythm");
  state.charts.rhythm = new Chart(document.getElementById("chart-rhythm"), {
    type: "line",
...
function renderEaster(data) {
  ...
  state.charts.easter = new Chart(document.getElementById("chart-easter"), {
    type: "bar",
  ...
  const services = (current && current.services) || [];
  destroyChart("easterServices");
  state.charts.easterServices = new Chart(document.getElementById("chart-easter-services"), {
    type: "bar",
    data: {
...
function renderChristmas(data) {
  ...
  state.charts.xmasTotals = new Chart(document.getElementById("chart-xmas-totals"), {
    type: "bar",
    data: {
  ...
  const services = (current && current.services) || [];
  destroyChart("xmasServices");
  state.charts.xmasServices = new Chart(document.getElementById("chart-xmas-services"), {
    type: "bar",
    data: {
      labels: services.map((s) => s.service_label),
      datasets: [
...
function renderStreaming(data) {
  ...
  destroyChart("streaming");
  state.charts.streaming = new Chart(document.getElementById("chart-streaming"), {
    type: "bar",
    data: {
