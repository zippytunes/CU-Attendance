...
function renderYearExplorer(data) {
  ...
  `;
  document.getElementById("year-weekly-title").textContent =
    `Sunday Attendance By Week — ${state.year}`;
...
function renderMonthExplorer(data) {
  ...
  document.getElementById("month-weekly-title").textContent =
    `Sunday Attendance By Week — ${m.month_name} ${m.year}`;
  document.getElementById("month-hour-title").textContent =
    `Sunday Monthly Average By Service — ${mh.month_name} ${mh.year}`;
...
function renderRhythm(data) {
  ...
  const scopeLabel = state.addOnline.rhythm
    ? `${serviceLabel(state.rhythmService)} + Online`
    : serviceLabel(state.rhythmService);
...
function setSectionCollapsed(section, collapsed) {
  ...
  if (!collapsed) {
    requestAnimationFrame(() => {
      ...
      requestAnimationFrame(() => resizeChartsIn(section));
    });
  }
}

function openCollapsibleSection(id, { scroll = true } = {}) {
...
function setupCollapsibleSections() {
  COLLAPSIBLE_IDS.forEach((id) => {
    ...
    section.classList.add("collapsible", "is-collapsed");
    ...
    if (first?.classList?.contains("section-head")) {
      h2 = first.querySelector("h2");
      const intro = first.querySelector(".section-intro");
      const rest = [...section.children].slice(1);
      if (intro) body.appendChild(intro);
      rest.forEach((el) => body.appendChild(el));
      first.remove();
    } else {
      h2 = section.querySelector(":scope > h2");
      [...section.children].forEach((el) => {
        if (el !== h2) body.appendChild(el);
      });
    }
  ...
  document.querySelectorAll('.toc a[href^="#"]').forEach((link) => {
    link.addEventListener("click", (e) => {
      ...
      e.preventDefault();
      openCollapsibleSection(id, { scroll: true });
      history.replaceState(null, "", `#${id}`);
    });
  });

  const hashId = location.hash.replace(/^#/, "");
  if (COLLAPSIBLE_IDS.includes(hashId)) {
    openCollapsibleSection(hashId, { scroll: true });
  }
}
